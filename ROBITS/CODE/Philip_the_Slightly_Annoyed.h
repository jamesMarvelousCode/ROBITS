...
const int left_speed = 80;//ov 75
const int rght_speed = 90;//ov 90
...
void loop() 
{ 
  //random number generator for use later
  RANUM = random(1, 100);
  ...
  //inches = microsecondsToInches(duration);//can remove????????????
...
// long microsecondsToInches(long microseconds)//can remove???????????????????????????
// {
